...
final class ContextTrackingUtils {
	...
	static final <V, T> Flux<V> trackContextLossForFlux(Flux<T> originalSource,
			ContextView ctxView,
		...
		Context lossDetectionKeyOnly = Context.of(key, true);
		Publisher<V> transformedByUser = transformer.apply(contextWithLossDetectionKey, Flux.from(sourceWithLossDetection));
		return Flux.from(transformedByUser)
		           .subscriberContext(lossDetectionKeyOnly);
	}
